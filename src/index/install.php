...
<head>
    ...
    if ($dockerMode) {
        ...
        if ($envHost && $envName && $envUser) {
            if ($envDbType === 'mysqli' && function_exists('mysqli_connect')) {
                $checkConn = @mysqli_connect($envHost, $envUser, $envPass);
                if ($checkConn && @mysqli_select_db($checkConn, $envName)) {
                    ...
                    if ($checkRes) {
                        ...
                        $installed = ((int)($checkRow['cnt'] ?? 0) >= 2);
                    }
                }
                if ($checkConn) mysqli_close($checkConn);
            } elseif (function_exists('pg_connect')) {
                $checkConn = @pg_connect("host=$envHost dbname=$envName user=$envUser password=$envPass");
                if ($checkConn) {
                    $checkRes = @pg_query($checkConn, "SELECT to_regclass('public.regnskab') AS regnskab, to_regclass('public.settings') AS settings");
                    if ($checkRes) {
                        $checkRow = pg_fetch_assoc($checkRes);
                        $installed = (!empty($checkRow['regnskab']) && !empty($checkRow['settings']));
                    }
                }
            }
        }
    ...
    function saldi_list_existing_tables($connection, $db_type) {
        $tables = array();
        if ($db_type == "mysqli") {
            ...
            $res = pg_query($connection, "SELECT tablename FROM pg_tables WHERE schemaname='public'");
            if ($res) {
    ...
    if (isset($_POST['opret'])){
        ...
        if ( strlen($db_password)==0 ) {
            if ($dockerMode && getenv('DB_PASSWORD')) {
        ...
        $_SESSION['db_password']=($dockerMode ? '' : $db_password);
...
#		($db_encode=="UTF8")? $href="INSTALLATION_utf8.txt":$href="INSTALLATION_lat9.txt";
        ...
        if (!$connection) die( "Kan ikke oprette forbindelse til database\n");

        $requiredTables = array('brugere','regnskab','online','kundedata','tekster','revisor','mysale','settings');
        $existingTables = saldi_list_existing_tables($connection, $db_type);
        $presentRequiredTables = array_intersect($requiredTables, $existingTables);
        if (count($presentRequiredTables) === count($requiredTables)) {
            $txt = "SALDI er allerede installeret i databasen $db_navn";
            alert($txt);
            print "<meta http-equiv=\"refresh\" content=\"0;URL=index.php\">";
            exit;
        } elseif (!empty($existingTables)) {
            $tableList = implode(", ", array_slice($existingTables, 0, 10));
            $txt = "Databasen $db_navn indeholder allerede tabeller ($tableList). Installationen afbrydes for at undgå overskrivning.";
            alert($txt);
...
#			print "<p><a href=../index/index.php>Forts&aelig;t</a></p>\n\n";
        ...
        if ($dockerMode) {
            $envDbType = strtolower(getenv('DB_TYPE') ?: 'postgresql');
            if (!$db_type) $db_type = ($envDbType === 'mysqli') ? 'MySQLi' : 'PostgreSQL';
            if (!$db_host) $db_host = getenv('DB_HOST') ?: $db_host;
            if (!$db_navn) $db_navn = getenv('DB_NAME') ?: $db_navn;
            if (!$db_bruger) $db_bruger = getenv('DB_USER') ?: $db_bruger;
            if (!$db_encode) $db_encode = getenv('DB_ENCODE') ?: $db_encode;
        }
