...
# --- Auto-generate connect.php if missing and env vars are present ---
if [ ! -f /var/www/html/includes/connect.php ]; then
    echo "Checking for environment variables to auto-generate connect.php..."
    
    # Check if required env vars are present
    if [ ! -z "$DB_HOST" ] && [ ! -z "$DB_USER" ] && [ ! -z "$DB_PASSWORD" ] && [ ! -z "$DB_NAME" ]; then
        echo "Generating includes/connect.php..."
        
        # Default values (db_type should be lowercase: postgresql or mysqli)
        DB_TYPE=${DB_TYPE:-postgresql}
        DB_ENCODE=${DB_ENCODE:-UTF8}
        
        # 1. Write the static part and Bash variables
...
EOF
        ...
        # 2. Append the PHP logic using quoted heredoc (PREVENTS BASH EXPANSION)
...
EOF
        ...
        # 3. Close the PHP tag
        echo "?>" >> /var/www/html/includes/connect.php

        # Set ownership to the calculated user
        chown www-data:www-data /var/www/html/includes/connect.php
    ...
    echo "connect.php already exists. Skipping generation."
fi
# ---------------------------------------------------------------------
